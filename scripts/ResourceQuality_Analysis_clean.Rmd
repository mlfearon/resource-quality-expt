...
require("knitr")
#opts_knit$set(root.dir = ("~/Dropbox (University of Michigan)/Manuscripts/FearonResourceQuality/2024/resource-quality-expt-main/"))
...
# Test if feeding rate predicts infection prevalence
p_prev_feed_model <- glmer(Prevalence/100 ~ Mean_Clearance_rel * Clone + (1|Diet), weights = Tested, family = "binomial", data = past_prev_sum)
p_prev_feed_model <- glmer(InfectionStatus ~ Clearance_rel.Week1 * Clone + (1|Diet) + (1|Block), family = "binomial", data = p_data_prev)
summary(p_prev_feed_model)
Anova(p_prev_feed_model)

# This is presented in Appendix S1.6
#p_prev_feed_model2 <- glmer(Prevalence/100 ~ Mean_Clearance * Clone + (1|Diet), weights = Tested, family = "binomial", data = past_prev_sum)
p_prev_feed_model2 <- glmer(InfectionStatus ~ Clearance.Week1 * Clone + (1|Diet) + (1|Block), family = "binomial", data = p_data_prev)
summary(p_prev_feed_model2)
Anova(p_prev_feed_model2)
...
m_prev_feed_model <- glmer(InfectionStatus ~ Clearance_rel.Week1 * Clone + (1|Diet) + (1|Block), family = "binomial", data = m_data_prev)
summary(m_prev_feed_model) 
Anova(m_prev_feed_model)
overdisp_fun(m_prev_feed_model)
library(rsq)
rsq.glmm(m_prev_feed_model)    #### NOTE FROM MAD: This line does not work for me
...
# These results presented in appendix S1.6
#m_prev_feed_model2 <- glmer(Prevalence/100 ~ Mean_Clearance_rel * Clone + (1|Diet), weights = Tested, family = "binomial", data = metsch_prev_sum)
m_prev_feed_model2 <- glmer(InfectionStatus ~ Clearance.Week1 * Clone + (1|Diet) + (1|Block), family = "binomial", glmerControl(optimizer = "bobyqa"), data = m_data_prev)
summary(m_prev_feed_model2) 
Anova(m_prev_feed_model2)
overdisp_fun(m_prev_feed_model2)
library(rsq)
rsq.glmm(m_prev_feed_model)     #### NOTE FROM MAD: This line does not work for me
...
ggsave(here("figures/FigS3_Prevalence_FeedingRate.tiff"), plot = FigS3, dpi = 600, width = 8, height = 3.5, units = "in", compression="lzw")
...
pmaturesporemod <- glmmTMB(mature_spores ~ Diet * Clone + (1|Block), family = nbinom2(), data = p_data_spores_mature) 
...
Anova(pmaturesporemod2)   # Table S1 in appendix
...
#### NOTE FROM MAD: This part isn't working, but I don't think we want this analysis anyway
...
p_maturespore_mc_fig <- plot(past_maturespores2, add.data = T) +
  scale_color_manual(values = diet_colors) +
  scale_fill_manual(values = diet_colors) +
  ggtitle("Pasteuria Spore Yield by microcystin + diet") +
  labs(x = "Microcystin conc (ug/L)", y = "log Bacterium Spore Yield") +
...
p_maturespore_mc_fig2 <- ggplot(p_data_spores_M_tox, aes(x = microcystin.avg+1, y = mature_spores)) +
  ...
  #xlim(0,20) +
  labs(x = "Average Microcystin Concentration (ug/L) log+1", y = "log Bacterium spore yield") +
  theme_classic() + 
  theme(axis.text = element_text(size=9, color = "black"), axis.title.y = element_text(size=11, color="black"), 
        axis.title.x = element_text(size=11, color="black"))
...
qqline(resid(sporesize_mod)) # looks ok, not the best   #### NOTE FROM MAD: This isn't running for me
...
qqline(resid(sporesize_mod2)) # looks ok, not the best   #### NOTE FROM MAD: This isn't running for me
...
#mmaturesporemod2 <- glmmTMB(mature_spores ~ microcystin.avg + Clone + Diet + (1|Block), family = nbinom2(), data = m_data_maturespores)  # Model can't run with clone included, probably because there are some diets missing for Mid37
mmaturesporemod2 <- glmmTMB(mature_spores ~ microcystin.avg + Diet + (1|Block), family = nbinom2(), data = m_data_maturespores)
overdisp_fun(mmaturesporemod2)
summary(mmaturesporemod2)
Anova(mmaturesporemod2)  # table S1 in appendix
AIC(mmaturesporemod2)
qqnorm(resid(mmaturesporemod2))
qqline(resid(mmaturesporemod2))      #### NOTE FROM MAD: This isn't running for me
...
# Remove other diets to only compare M and M+ diets  (3 points in the data...)
m_data_spores_M_tox <- filter(m_data_maturespores, Diet == "M" | Diet == "M+")
mmaturesporemod4 <- glmmTMB(mature_spores ~ microcystin.avg, family = nbinom2(), data = m_data_spores_M_tox)  
summary(mmaturesporemod4)
Anova(mmaturesporemod4)   #### NOTE FROM MAD: This isn't running for me
qqnorm(resid(mmaturesporemod4))
qqline(resid(mmaturesporemod4))   #### NOTE FROM MAD: This isn't running for me
...
# Marginal effects from best model with Abundance on the x axis
   #### NOTE FROM MAD: This isn't running for me but I think we aren't using this analysis; skipping this whole section
...
mimmaturesporemod <- glmmTMB(round(immature_spores) ~ Diet + Clone + (1|Block), family = nbinom1(), data = m_data_spores)
summary(mimmaturesporemod)
Anova(mimmaturesporemod)
AIC(mimmaturesporemod)
testDispersion(mimmaturesporemod)
testZeroInflation(mimmaturesporemod)
mimmaturespore_simResid <- simulateResiduals(fittedModel = mimmaturesporemod)
plot(mimmaturespore_simResid)

mimmaturesporemod_contrasts <- emmeans(mimmaturesporemod, specs = pairwise ~ Diet | Clone, type = "response")
mimmaturesporemod_contrasts   #### NOTE FROM MAD: This isn't running for me
...
mbudsporemod <- glmmTMB(round(bud_spores) ~ Diet + Clone + (1|Block), family = nbinom2(), data = m_data_spores)
mbudsporemod <- glmmTMB(round(bud_spores) ~ Diet + Clone + (1|Block), family = nbinom1(), data = m_data_spores)  # better model
summary(mbudsporemod)
Anova(mbudsporemod)
AIC(mbudsporemod)
testDispersion(mbudsporemod)
testZeroInflation(mbudsporemod)
mbudspore_simResid <- simulateResiduals(fittedModel = mbudsporemod)
plot(mbudspore_simResid)


mbudsporemod_contrasts <- emmeans(mbudsporemod, specs = pairwise ~ Diet | Clone, type = "response")
mbudsporemod_contrasts    #### NOTE FROM MAD: This isn't running for me
...
#mgutsporemod <- glmmTMB(TotalGutSpores ~ Diet * Clone, family = nbinom2(), data = m_data_gutspore)   # higher AIC
#mgutsporemod <- glmmTMB(TotalGutSpores ~ Diet * Clone, family = nbinom1(), data = m_data_gutspore)  # higher AIC
...
# Hemocytes and including body size as a predictor
...
# Hemocytes per spore and NOT including body size as a predictor [USE THIS ONE IN THE MANUSCRIPT]
...
mprevmod2 <- glm(InfectionStatus ~ TotalPunctSpores + HemocytesPerSpore, family = "binomial", data = m_data_inf)
summary(mprevmod2)  # try putting diet and/or clone as a random effect?
Anova(mprevmod2)
overdisp_fun(mprevmod2)
testDispersion(mprevmod2)
testZeroInflation(mprevmod2)
mprevmod2_simResid <- simulateResiduals(fittedModel = mprevmod2)
plot(mprevmod2_simResid)


# Model with diet and clone included does not perform well because there is too much separation in the data due to the effects of diet. 
...
# Marginal effects from best model with Abundance on the x axis
metsch_inf <- ggpredict(mprevmod2, c("TotalPunctSpores", "Diet", "Clone"))   #### NOTE FROM MAD: This isn't running for me
...
p_data_fecund <- p_data %>% filter(!is.na(Total.Babies)  & lifespan.days > 11)  # first babies were observed on day 11 
...
summary(pfecundmod)
Anova(pfecundmod)        # Table S2 in appendix
...
# figure for presentation
p_fecund_tot2 <- ggplot(p_data_fecund, aes(x = Infection, y = Total.Babies)) +
...
m_data_fecund <- m_data %>% filter(!is.na(Total.Babies) & lifespan.days > 11)  # first babies observed on day 11
...
summary(mfecundmod)
Anova(mfecundmod) # model doesn't have enough power for the 3 way interaction, Table S2
AIC(mfecundmod)
...
mfecundmod_contrasts3 <- emmeans(mfecundmod, specs = pairwise ~ Clone, type = "response")

# test of microcystin effect on offspring production
m_data_fecund_toxintest <- filter(m_data_fecund, Diet == "M" | Diet == "M+")
mfecundmod2 <- glmmTMB(Total.Babies ~ microcystin.avg + Infection + Clone + (1|Block), family = nbinom1(), data = m_data_fecund_toxintest)
summary(mfecundmod2)
Anova(mfecundmod2)   # technically rank deficient to handle interactions, so removed all interactions
overdisp_fun(mfecundmod2)
AIC(mfecundmod2)
testDispersion(mfecundmod2)
testZeroInflation(mfecundmod2)
mfecund2_simResid <- simulateResiduals(fittedModel = mfecundmod2)
plot(mfecund2_simResid)
```

```{r, total fecunity by treatment for metsch}
m_fecund_tot2 <- ggplot(m_data_fecund, aes(x = Infection, y = Total.Babies)) +
...
# Little r for infection status x clone
p_little_r3 <- ggplot(p_lifetable_clone_inf, aes(x = Infection, y = r_mx)) +
...
ggsave("figures/PastLittleR_ClonexInfection.tiff", plot = p_little_r3, dpi = 600, width = 6.5, height = 4, units = "in", compression="lzw")
...
# Little r for infection status x clone
m_little_r3 <- ggplot(m_lifetable_clone_inf, aes(x = Infection, y = r_mx)) +
  geom_point(aes(), size=2, position=position_dodge(width=0.5), show.legend = F) +
  geom_errorbar(aes(ymin = r_mx-Std.error_r_mx, ymax=r_mx+Std.error_r_mx), width = 0.2,  position=position_dodge(width=0.5), show.legend = F) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "lightgray") +
  facet_wrap(~Clone) +
...
ggsave("figures/MetschLittleR_ClonexInfection.tiff", plot = m_little_r3, dpi = 600, width = 6.5, height = 4, units = "in", compression="lzw")
...
## Body size differences during Week 1 (parasite exposure) for Pasteuria expt

```{r, model and fig of body size diff during exposure for Past expt}
...
p_size <- ggplot(p_data_size, aes(x = Infection, y = Length_Week1)) +
  ...
  scale_color_manual(values = c(rep("black", 4))) +
  ggtitle("Bacteria Experiment") +
  labs(x = "Infection Status", y = bquote("Body Size During Parasite Exposure ("* mu* "m)")) +
  theme_classic() + 
  theme(axis.text = element_text(size=9, color = "black"), axis.title.y = element_text(size=11, color="black"), 
        axis.title.x = element_text(size=11, color="black"))
p_size
...
# figures over time
## Plot of body size over time by week
...
```{r, model and fig of body size diff during exposure for Metsch expt}
...
```{r, Figure S4 for growth rate}

FigureS5 <- ggarrange(m_size, p_size, plot_byWeek_size_metsch, plot_byWeek_size_past, m_growth2, p_growth2, labels = "auto", ncol = 2, nrow = 3, widths = c(3.3,4))
FigureS5
ggsave("figures/FigS5_GrowthRate_BodySize.tiff", plot = FigureS5, dpi = 600, width = 8.5, height = 11, units = "in", compression="lzw")
...
summary(pfeedmod)
Anova(pfeedmod)  # Appendix Table S2
AIC(pfeedmod)
plot(pfeedmod)
...
#pfeedmod_contrasts4 <- emmeans(pfeedmod2, specs = pairwise ~ Diet | Infection, type = "response")
pfeedmod_contrasts 
...
summary(pfeedmod2)
Anova(pfeedmod2)   # Appendix Table S2
AIC(pfeedmod2)
plot(pfeedmod2)
...
pfeedmod2_contrasts2   # This result tells us that there is a sig difference between uninfected and exposed for Standard but NOT for Mid37 (same for both feedingand relative feeding)

# contrasts of diet effects by each infection status (averaged over both clones)
pfeedmod_contrasts4 <- emmeans(pfeedmod2, specs = pairwise ~ Diet | Infection + Clone, type = "response")
#pfeedmod_contrasts4 <- emmeans(pfeedmod2, specs = pairwise ~ Diet | Infection, type = "response")
pfeedmod_contrasts4   # Mid37 there is a dif between S - M for uninfected and SM - M+ for exposed, and no differences for infected; and for Std uninfected there are dif between S - M, S - M+, and SM - M, for exposed and infected dif between S - M for both
...
p_feeding_fig
p_feeding_fig +
  geom_point(data = pfeedmod_infstatus_est, aes(x = Infection, y = emmean), size = 3, color = "black") +
  geom_errorbar(data = pfeedmod_infstatus_est, aes(x = Infection, ymin = lower.CL, ymax = upper.CL), width =0.1) +
  facet_wrap(~Clone)
ggsave("figures/PastFeedingRate_DietxClonexInfection.tiff", plot = p_feeding_fig, dpi = 600, width = 5, height = 4, units = "in", compression="lzw")


# figure of feeding rate by infection and clone (averaged over all diets)
p_feeding_inf_fig <- ggplot(data = pfeedmod_infstatus_est, aes(x = Infection, y = emmean)) +
...
# figure with bodysize corrected feeding rate
...
```
 ...
 The relative body size feeding rate seems like the better model because it controls for differences between clones that are likely just due to body size differences, while diet effects are sig in both models indicating that it high quality diet fed animals are eating more relative to their bodysize.
...
mfeedmod <- lmer(Clearance.Week1 ~ Diet * Infection * Clone +(1|Block), data = m_data_feeding)
...
# figure for presentation bodysize corrected feeding rate
...
```{r, Figure 1 and 2 of Metsh and Past infection prevalence, feeding and exposure rate, spore yield, spore size an gut spores}
...
Figure1
ggsave("figures/Fig1_Prev_Exposure_Yield.tiff", plot = Figure1, dpi = 600, width = 7.5, height = 9, units = "in", compression="lzw")



Figure2 <- ggarrange(m_totgutspores_fig3, sporesize_plot2, m_hemocytesperspore_fig3, labels = "auto",
                      ncol = 2, nrow = 2, legend = "right", common.legend = T)
Figure2
ggsave("figures/Fig2_GutSpores_Hemo_SporeSize.tiff", plot = Figure2, dpi = 600, width = 7, height = 7, units = "in", compression="lzw")
...
```{r, Figure 3 of feeding rate and fecundity for fungus and bacteria experiments}


Figure3 <- ggarrange(m_little_r2, p_little_r2, m_little_r3, p_little_r3, m_survival_fig, p_survival_fig, labels = "auto",
                      ncol = 2, nrow = 3, heights = c(3.2,3,3), legend = "right", common.legend = T)
Figure3
ggsave("figures/Fig3_LittleR_Survival.tiff", plot = Figure3, dpi = 600, width = 8, height = 9, units = "in", compression="lzw")
 widths = c(3.3,4)

FigureS4 <- ggarrange(m_fecund_tot2, p_fecund_tot2, labels = "auto",
                      ncol = 2, nrow = 1, legend = "right", common.legend = T)
ggsave("figures/FigS4_Fecundity.tiff", plot = FigureS4, dpi = 600, width = 8.5, height = 3.5, units = "in", compression="lzw")


LittleR_clone_inf <- ggarrange(m_little_r3, p_little_r3, labels = "auto",
                      ncol = 2, nrow = 1)
ggsave("figures/LitteR_ClonexInfectionStatus.tiff", plot = LittleR_clone_inf, dpi = 600, width = 8.5, height = 3.5, units = "in", compression="lzw")
...
# Respiration rate over whole experiment (DON"T USE THIS MDOEL, doesn't work)
#prespmod2 <- lmer(log(metabolic.rate+1) ~ Diet * Infection + Clone + (1|Week) + (1|Block), data = p_longdata_resp)
summary(prespmod2)
Anova(prespmod2)
AIC(prespmod2)
...
shapiro.test(resid(prespmod2))  ## very significant
...
filter(m_data, metabolic.rate.Week1 > 0.006)
# Respiration rate during week 1 (exposure period) only
...
# Respiration rate over whole experiment (DON'T USE THIS MODEL, DOESN'T WORK)
...
```{r, Figure S5 for metabolic rates}

FigureS6 <- ggarrange(m_resp_fig2, p_resp_fig2, plot_byWeek_resp_metsch, plot_byWeek_resp_past, labels = "auto",
                      ncol = 2, nrow = 2, widths = c(3.3,4))
FigureS6
ggsave("figures/FigS6_MetabolicRate.tiff", plot = FigureS6, dpi = 600, width = 8.5, height = 7, units = "in", compression="lzw")
...
#### Analysis and figure for the manuscript
# both clones
...
ggforest(cox.m_diet_infstatus_mid2, data = p_data_mid)  # use this for the mid37 specific model
...
```{r, pasteuria lifespan figure, fig.width=15, fig.height=15}
...
```{r, metsch lifespan figure, fig.width=15, fig.height=15}
...
```{r, make a data frame and figure of the percent change between uninfected and exposed by clone for all host metrics}
# Extract Bacteria fecundity means and p-values
df_pfecund <- data.frame(
  Metric = c("Fecundity", "Fecundity"),
  Experiment = c(rep("Bacteria", 2)),
  Clone = c("Mid37", "Standard"),
  Uninfected = c(as.data.frame(pfecundmod_contrasts2$emmeans)$response[1], as.data.frame(pfecundmod_contrasts2$emmeans)$response[4]),
  Exposed = c(as.data.frame(pfecundmod_contrasts2$emmeans)$response[2], as.data.frame(pfecundmod_contrasts2$emmeans)$response[5]),
  Infected = c(as.data.frame(pfecundmod_contrasts2$emmeans)$response[3], as.data.frame(pfecundmod_contrasts2$emmeans)$response[6]),
  U_E_pvalue = c(as.data.frame(pfecundmod_contrasts2$contrasts)$p.value[1], as.data.frame(pfecundmod_contrasts2$contrasts)$p.value[4])
)

# Extract Fungus fecundity means and p-values
df_mfecund <- data.frame(
  Metric = c("Fecundity", "Fecundity"),
  Experiment = c(rep("Fungus", 2)),
  Clone = c("Mid37", "Standard"),
  Uninfected = c(as.data.frame(mfecundmod_contrasts2$emmeans)$response[1], as.data.frame(mfecundmod_contrasts2$emmeans)$response[4]),
  Exposed = c(as.data.frame(mfecundmod_contrasts2$emmeans)$response[2], as.data.frame(mfecundmod_contrasts2$emmeans)$response[5]),
  Infected = c(as.data.frame(mfecundmod_contrasts2$emmeans)$response[3], as.data.frame(mfecundmod_contrasts2$emmeans)$response[6]),
  U_E_pvalue = c(as.data.frame(mfecundmod_contrasts2$contrasts)$p.value[1], as.data.frame(mfecundmod_contrasts2$contrasts)$p.value[4])
)


# Extract Bacteria growth rate means and p-values
df_pgrowth <- data.frame(
  Metric = c("Growth Rate", "Growth Rate"),
  Experiment = c("Bacteria", "Bacteria"),
  Clone = c("Mid37", "Standard"),
  Uninfected = c(as.data.frame(pgrowthmod_contrasts2$emmeans)$emmean[1], as.data.frame(pgrowthmod_contrasts2$emmeans)$emmean[4]),
  Exposed = c(as.data.frame(pgrowthmod_contrasts2$emmeans)$emmean[2], as.data.frame(pgrowthmod_contrasts2$emmeans)$emmean[5]),
  Infected = c(as.data.frame(pgrowthmod_contrasts2$emmeans)$emmean[3], as.data.frame(pgrowthmod_contrasts2$emmeans)$emmean[6]),
  U_E_pvalue = c(as.data.frame(pgrowthmod_contrasts2$contrasts)$p.value[1], as.data.frame(pgrowthmod_contrasts2$contrasts)$p.value[4])
)

# Extract Fungus growth rate means and p-values
df_mgrowth <- data.frame(
  Metric = c("Growth Rate", "Growth Rate"),
  Experiment = c("Fungus", "Fungus"),
  Clone = c("Mid37", "Standard"),
  Uninfected = c(as.data.frame(mgrowthmod_contrasts2$emmeans)$emmean[1], as.data.frame(mgrowthmod_contrasts2$emmeans)$emmean[4]),
  Exposed = c(as.data.frame(mgrowthmod_contrasts2$emmeans)$emmean[2], as.data.frame(mgrowthmod_contrasts2$emmeans)$emmean[5]),
  Infected = c(as.data.frame(mgrowthmod_contrasts2$emmeans)$emmean[3], as.data.frame(mgrowthmod_contrasts2$emmeans)$emmean[6]),
  U_E_pvalue = c(as.data.frame(mgrowthmod_contrasts2$contrasts)$p.value[1], as.data.frame(mgrowthmod_contrasts2$contrasts)$p.value[4])
)


# Extract Bacteria feeding rate means and p-values
df_pfeed <- data.frame(
  Metric = c("Feeding Rate", "Feeding Rate"),
  Experiment = c("Bacteria", "Bacteria"),
  Clone = c("Mid37", "Standard"),
  Uninfected = c(as.data.frame(pfeedmod_contrasts2$emmeans)$emmean[1], as.data.frame(pfeedmod_contrasts2$emmeans)$emmean[4]),
  Exposed = c(as.data.frame(pfeedmod_contrasts2$emmeans)$emmean[2], as.data.frame(pfeedmod_contrasts2$emmeans)$emmean[5]),
  Infected = c(as.data.frame(pfeedmod_contrasts2$emmeans)$emmean[3], as.data.frame(pfeedmod_contrasts2$emmeans)$emmean[6]),
  U_E_pvalue = c(as.data.frame(pfeedmod_contrasts2$contrasts)$p.value[1], as.data.frame(pfeedmod_contrasts2$contrasts)$p.value[4])
)

# Extract Fungus feeding rate means and p-values
df_mfeed <- data.frame(
  Metric = c("Feeding Rate", "Feeding Rate"),
  Experiment = c("Fungus", "Fungus"),
  Clone = c("Mid37", "Standard"),
  Uninfected = c(as.data.frame(mfeedmod_contrasts2$emmeans)$emmean[1], as.data.frame(mfeedmod_contrasts2$emmeans)$emmean[4]),
  Exposed = c(as.data.frame(mfeedmod_contrasts2$emmeans)$emmean[2], as.data.frame(mfeedmod_contrasts2$emmeans)$emmean[5]),
  Infected = c(as.data.frame(mfeedmod_contrasts2$emmeans)$emmean[3], as.data.frame(mfeedmod_contrasts2$emmeans)$emmean[6]),
  U_E_pvalue = c(as.data.frame(mfeedmod_contrasts2$contrasts)$p.value[1], as.data.frame(mfeedmod_contrasts2$contrasts)$p.value[4])
)


# Extract Bacteria metabolic rate means and p-values
df_presp <- data.frame(
  Metric = c("Metabolic Rate", "Metabolic Rate"),
  Experiment = c("Bacteria", "Bacteria"),
  Clone = c("Mid37", "Standard"),
  Uninfected = c(as.data.frame(prespmod_contrasts2$emmeans)$emmean[1], as.data.frame(prespmod_contrasts2$emmeans)$emmean[4]),
  Exposed = c(as.data.frame(prespmod_contrasts2$emmeans)$emmean[2], as.data.frame(prespmod_contrasts2$emmeans)$emmean[5]),
  Infected = c(as.data.frame(prespmod_contrasts2$emmeans)$emmean[3], as.data.frame(prespmod_contrasts2$emmeans)$emmean[6]),
  U_E_pvalue = c(as.data.frame(prespmod_contrasts2$contrasts)$p.value[1], as.data.frame(prespmod_contrasts2$contrasts)$p.value[4])
)

# Extract Fungus metabolic rate means and p-values
df_mresp <- data.frame(
  Metric = c("Metabolic Rate", "Metabolic Rate"),
  Experiment = c("Fungus", "Fungus"),
  Clone = c("Mid37", "Standard"),
  Uninfected = c(as.data.frame(mrespmod_contrasts2$emmeans)$emmean[1], as.data.frame(mrespmod_contrasts2$emmeans)$emmean[4]),
  Exposed = c(as.data.frame(mrespmod_contrasts2$emmeans)$emmean[2], as.data.frame(mrespmod_contrasts2$emmeans)$emmean[5]),
  Infected = c(as.data.frame(mrespmod_contrasts2$emmeans)$emmean[3], as.data.frame(mrespmod_contrasts2$emmeans)$emmean[6]),
  U_E_pvalue = c(as.data.frame(mrespmod_contrasts2$contrasts)$p.value[1], as.data.frame(mrespmod_contrasts2$contrasts)$p.value[4])
)


## little r calculation
# p_littleR <- p_lifetable %>%
#   dplyr::group_by(Clone, Infection)  %>%
#   dplyr::summarize(r_mx = mean(r_mx)) %>%
#   pivot_wider(names_from = Infection, values_from = r_mx) %>%
#   mutate(Metric = "Pop Growth", Experiment = "Bacteria", U_E_pvalue = NA) %>%
#   dplyr::select(Metric, Experiment, Clone, Uninfected, Exposed, Infected, U_E_pvalue)


p_littleR <- p_lifetable_clone_inf %>%
  dplyr::select(Clone, Infection, r_mx) %>%
  dplyr::group_by(Clone, Infection) %>%
  pivot_wider(names_from = Infection, values_from = r_mx) %>%
  mutate(Metric = "Pop Growth", Experiment = "Bacteria") %>%  # note this not the p-value but just a place holder to indicate that the standard errors do not overlap between uninfected and exposed groups in mid37s, but do overlap for standards
  dplyr::select(Metric, Experiment, Clone, Uninfected, Exposed, Infected)
p_littleR$U_E_pvalue <- c(0.01, 0.2)
  
# m_littleR <- m_lifetable %>%
#   dplyr::group_by(Clone, Infection)  %>%
#   dplyr::summarize(r_mx = mean(r_mx)) %>%
#   pivot_wider(names_from = Infection, values_from = r_mx) %>%
#   mutate(Metric = "Pop Growth", Experiment = "Fungus", U_E_pvalue = NA) %>%
#   dplyr::select(Metric, Experiment, Clone, Uninfected, Exposed, Infected, U_E_pvalue)

m_littleR <- m_lifetable_clone_inf %>%
  dplyr::select(Clone, Infection, r_mx) %>%
  dplyr::group_by(Clone, Infection) %>%
  pivot_wider(names_from = Infection, values_from = r_mx) %>%
  mutate(Metric = "Pop Growth", Experiment = "Fungus", U_E_pvalue = 0.01) %>%  # note this not the p-value but just a place holder to indicate that the standard errors do not overlap between uninfected and exposed groups
  dplyr::select(Metric, Experiment, Clone, Uninfected, Exposed, Infected, U_E_pvalue)


### Bind all data sets together (except for mortality data)
dif_data <- rbind(df_mfeed, df_pfeed, df_mgrowth, df_pgrowth, df_mfecund, df_pfecund, m_littleR, p_littleR, df_mresp, df_presp)

# Calculate the percent change between uninfected and exposed, make a logical sig variable
dif_data <- dif_data %>%
  mutate(U_E_sig = if_else(U_E_pvalue < 0.05, T, F, missing = F),
         Percent_change = ((Exposed - Uninfected)/Uninfected),
         sig_fill_values = paste(Experiment, U_E_sig, sep = "_"))
dif_data$Metric <- factor(dif_data$Metric, levels = c("Feeding Rate", "Growth Rate", "Metabolic Rate", "Fecundity", "Pop Growth"))


# Mortality differences

# Extract Bacteria survival rate differences and p-values
df_pmortality <- data.frame(
  Metric = c("Mortality Rate", "Mortality Rate"),
  Experiment = c("Bacteria", "Bacteria"),
  Clone = c("Mid37", "Standard"),
  Uninfected = c("Reference", "Reference"),
  Exposed = c(exp(cox.diet_infstatus_mid2$coefficients)[4], exp(cox.diet_infstatus_std2$coefficients)[4]),
  Infected = c(exp(cox.diet_infstatus_mid2$coefficients)[5], exp(cox.diet_infstatus_std2$coefficients)[5]),
  U_E_pvalue = c(summary(cox.diet_infstatus_mid2)$coefficients[4,5], summary(cox.diet_infstatus_std2)$coefficients[4,5])
)

# Extract Fungus survival rate differences and p-values
df_mmortality <- data.frame(
  Metric = c("Mortality Rate", "Mortality Rate"),
  Experiment = c("Fungus", "Fungus"),
  Clone = c("Mid37", "Standard"),
  Uninfected = c("Reference", "Reference"),
  Exposed = c(exp(cox.m_diet_infstatus_mid2$coefficients)[4], exp(cox.m_diet_infstatus_std2$coefficients)[4]),
  Infected = c(exp(cox.m_diet_infstatus_mid2$coefficients)[5], exp(cox.m_diet_infstatus_std2$coefficients)[5]),
  U_E_pvalue = c(summary(cox.m_diet_infstatus_mid2)$coefficients[4,5], summary(cox.m_diet_infstatus_std2)$coefficients[4,5])
)

mortdif_data <- rbind(df_pmortality, df_mmortality)
mortdif_data <- mutate(mortdif_data, U_E_sig = if_else(U_E_pvalue < 0.05, T, F), sig_fill_values = paste(Experiment, U_E_sig, sep = "_"))



## Figure of the percent dif between uninf and exposed for each metric, parasite experiment, and clone

percentdif_fig <- ggplot(dif_data, aes(x = Metric, y = Percent_change, color = Experiment, fill = sig_fill_values, shape = Clone)) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray") +
  geom_point(position = position_dodge(width = 0.5), size = 2.5) +
  labs(x = "Host Metric", y = "Percent change between Uninfected and Exposed") +
  scale_y_continuous(breaks = c(-1,-0.5,0,0.5,1,1.5,2,2.5,3,3.5,4,4.5,5,5.5,6,6.5,7), labels = c("-100%","","0%", "","100%","","200%","","300%", "","400%", "","500%","","600%","","700%")) +
  scale_shape_manual(values = c(21,22)) +
  scale_color_manual(values = c("#D16103", "#4E84C4")) +
  scale_fill_manual(values = c("white", "#D16103", "white", "#4E84C4")) +
  guides(fill = "none") +
  theme_classic() + 
  theme(axis.text = element_text(size=9, color = "black"), axis.title.y = element_text(size=10, color="black"), 
        axis.title.x = element_text(size=11, color="black"))

percentdif_fig


mortdif_fig <- ggplot(mortdif_data, aes(x = Metric, y = Exposed, color = Experiment, fill = sig_fill_values, shape = Clone)) +
  geom_hline(yintercept = 1, linetype = "dashed", color = "gray") +
  geom_point(position = position_dodge(width = 0.5), size = 2.5) +
  labs(x = "Host Metric", y = "Fold difference between Uninfected and Exposed") +
  scale_y_continuous(breaks = c(1, 5, 10, 15, 20, 25), ) +
  scale_shape_manual(values = c(21,22)) +
  scale_color_manual(values = c("#D16103", "#4E84C4")) +
  scale_fill_manual(values = c("white", "#D16103", "#4E84C4")) +
  guides(fill = "none") +
  theme_classic() + 
  theme(axis.text = element_text(size=9, color = "black"), axis.title.y = element_text(size=10, color="black"), 
        axis.title.x = element_text(size=11, color="black"))
...
Figure4 <- ggarrange(percentdif_fig, mortdif_fig, nrow = 1, ncol = 2, labels = "auto", common.legend = T, legend = "right", widths = c(3,1))
Figure4

ggsave("figures/Fig4_PercentChangeUninfExposed.tiff", plot = Figure4, dpi = 600, width = 8, height = 4, units = "in", compression="lzw")
