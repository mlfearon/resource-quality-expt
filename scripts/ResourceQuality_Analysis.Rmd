...
mmaturesporemod2 <- glmmTMB(mature_spores ~ microcystin.avg + Clone + Diet + (1|Block), family = nbinom2(), data = m_data_maturespores)  # Model can't run with clone incldued, probably because there are some diets missing for Mid37
...
pfecundmod <- glmmTMB(Total.Babies ~ Diet * Infection * Clone + (1|Block), family = nbinom2(), data = p_data_fecund)  #lower AIC than poisson and quasipoisson
summary(pfecundmod)
Anova(pfecundmod)
...
pfecundmod2 <- glmmTMB(Total.Babies ~ log(microcystin.avg+1) * Infection * Clone + (1|Block), family = nbinom2(), data = p_data_fecund_toxintest)
summary(pfecundmod2)
Anova(pfecundmod2)
...
p_fecund_tot2 <- ggplot(p_data_fecund, aes(x = Infection, y = Total.Babies)) +
  geom_boxplot(aes(fill=Diet),position=position_dodge(width=0.8), show.legend = F) +
...
mfecundmod <- glmmTMB(Total.Babies ~ Diet + Infection + Clone + Diet:Infection + Diet:Clone + Infection:Clone + (1|Block), family = nbinom1(), data = m_data_fecund)   # lower AIC than neg binomial and corrects for overdispersion
summary(mfecundmod)
...
p_little_r2 <- ggplot(p_lifetable, aes(x = Infection, y = r_mx)) +
  ...
  theme_classic() + 
  theme(axis.text = element_text(size=9, color = "black"), axis.title.y = element_text(size=11, color="black"), 
        axis.title.x = element_text(size=9, color="black"), legend.position = c(0.9,0.3))
p_little_r2
...
```{r, Figure S3 for growth rate}

FigureS3 <- ggarrange(m_size, p_size, plot_byWeek_size_metsch, plot_byWeek_size_past, m_growth2, p_growth2, labels = "auto", ncol = 2, nrow = 3, widths = c(3.3,4))
FigureS3
ggsave("figures/FigS3_GrowthRate_BodySize.tiff", plot = FigureS3, dpi = 600, width = 8.5, height = 11, units = "in", compression="lzw")
...
pfeedmod_emm <- emmeans(pfeedmod2, ~ Diet * Infection * Clone)
pfeedmod_contrasts <- pairs(pfeedmod_emm, simple = "each")
pfeedmod_contrasts

# contrasts of diet effects by each clone (averaged over all infection statuses)
pfeedmod_contrasts2 <- emmeans(pfeedmod2, specs = pairwise ~ Diet | Clone, type = "response")
pfeedmod_contrasts2   # Mid37: S > M; SM > M and M+; Std: S > M and M+, SM > M

# contrasts of infection status effects by each clone (averaged over all diets)
pfeedmod_contrasts3 <- emmeans(pfeedmod2, specs = pairwise ~ Infection | Clone, type = "response")
pfeedmod_contrasts3 <- emmeans(pfeedmod2, specs = pairwise ~ Infection, type = "response")
pfeedmod_contrasts3   # This result tells us that there is a sig difference between uninfected and exposed for Standard but NOT for Mid37 (same for both feedingand relative feeding)
...
# tests contrasts for each factor holding the other two constant in separate tests. 
mfeedmod_emm <- emmeans(mfeedmod2, ~ Diet + Infection + Clone + Diet:Infection + Diet:Clone + Infection:Clone)
mfeedmod_contrasts <- pairs(mfeedmod_emm, simple = "each")
mfeedmod_contrasts

# contrasts of diet effects by each clone (averaged over all infection statuses)
mfeedmod_contrasts2 <- emmeans(mfeedmod2, specs = pairwise ~ Diet | Clone, type = "response")
mfeedmod_contrasts2   # Mid37: S > M; SM > M and M+; Std: S > M and M+, SM > M

# contrasts of infection status effects by each clone (averaged over all diets)
mfeedmod_contrasts3 <- emmeans(mfeedmod2, specs = pairwise ~ Infection | Clone, type = "response")
mfeedmod_contrasts3 <- emmeans(mfeedmod2, specs = pairwise ~ Clone | Infection, type = "response")
mfeedmod_contrasts3 <- emmeans(mfeedmod2, specs = pairwise ~ Infection, type = "response")
mfeedmod_contrasts3   # This result tells us that there is a sig difference between uninfected and exposed for Standard but NOT for Mid37 (same for both feedingand relative feeding)

# contrasts of diet effects by each infection status (averaged over both clones)
mfeedmod_contrasts4 <- emmeans(mfeedmod2, specs = pairwise ~ Diet | Infection + Clone, type = "response")
mfeedmod_contrasts4 <- emmeans(mfeedmod2, specs = pairwise ~ Diet | Infection, type = "response")
...
Figure3 <- ggarrange(m_feeding3, p_feeding3, m_fecund_tot2, p_fecund_tot2, m_little_r2, p_little_r2, labels = "auto",
                      ncol = 2, nrow = 3, legend = "right", common.legend = T)
Figure3
ggsave("figures/Fig3_Feeding_Fecund_Survival.tiff", plot = Figure3, dpi = 600, width = 8.5, height = 10, units = "in", compression="lzw")
...
```{r, Figure S4 for metabolic rates}

FigureS4 <- ggarrange(m_resp_fig2, p_resp_fig2, plot_byWeek_resp_metsch, plot_byWeek_resp_past, labels = "auto",
                      ncol = 2, nrow = 2, widths = c(3.3,4))
FigureS4
ggsave("figures/FigS4_MetabolicRate.tiff", plot = FigureS4, dpi = 600, width = 8.5, height = 7, units = "in", compression="lzw")
