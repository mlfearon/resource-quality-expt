...
p_maturespore_fig2 <- ggplot(p_data_spores_mature, aes(x = Diet, y = mature_spores)) +
  ...
  #scale_y_log10(labels = scales::comma, breaks = c(100, 1000, 10000,100000,1000000)) +
  #ggtitle("Mature Pasteuria Spore Yield by diet x host clone") +
  labs(x = "Diet Treatment", y = "Pasteuria spore yield (log scaled)") +
  theme_classic() +
  theme(axis.text = element_text(size=9, color = "black"), axis.title.y = element_text(size=11, color="black"), 
        axis.title.x = element_text(size=11, color="black"))
p_maturespore_fig2
...
Anova(mgutsporemod)      ## Use this model for results in the mansuscript
...
mhemocytemod3 <- glmmTMB(Hemocytes ~ Diet + Clone + length.um + Diet:length.um + (1|Unique.code), offset = log(TotalPunctSpores), family = poisson(), data = m_data_hemocytes)
# Note: left off the block random effect because all these animals were in the same block
...
Figure2 <- ggarrange(p_prev_fig3, p_maturespore_fig2, sporesize_plot2, labels = "auto",
                      ncol = 2, nrow = 2, widths = c(3,3,4,2))
Figure2
...
shapiro.test(resid(pfeedmod))   # significant! 
