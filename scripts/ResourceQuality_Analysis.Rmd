...
past_prev_sum <- p_data_prev %>%
  group_by(Diet, Clone) %>%
  summarise(Infected = sum(InfectionStatus), Tested = length(Rep), Mean_Clearance_rel = mean(Clearance_rel.Week1, na.rm = T), .groups = "rowwise") %>%
...
p_prev_feed_fig <- ggplot(past_prev_sum, aes(x = Mean_Clearance_rel, y= Prevalence)) +
  ...
  theme(axis.text = element_text(size=9, color = "black"), axis.title.y = element_text(size=11, color="black"), 
        axis.title.x = element_text(size=9, color="black"))
p_prev_feed_fig
ggsave(here("figures/PasteuriaPrevCalcualtions_RelFeedingRate.tiff"), plot = p_prev_feed_fig, dpi = 600, width = 4, height = 3, units = "in", compression="lzw")
...
# test a model with only S and SM diets to remove treatments with no infection (except Std M+ did have a little infection)
m_data_prev2 <- filter(m_data_prev, Diet == "S" | Diet == "SM")

mprevmod2 <- glmer(InfectionStatus ~ Diet * Clone + (1|Block), family = binomial, data = m_data_prev2)
summary(mprevmod2)  # block random effect does work in this model
Anova(mprevmod2)
...
m_prev_feed_fig <- ggplot(metsch_prev_sum, aes(x = Mean_Clearance_rel, y= Prevalence)) +
  ...
  theme(axis.text = element_text(size=9, color = "black"), axis.title.y = element_text(size=11, color="black"), 
        axis.title.x = element_text(size=9, color="black"))
m_prev_feed_fig
ggsave(here("figures/MetschPrevCalcualtions_RelFeedingRate2.tiff"), plot = m_prev_feed_fig, dpi = 600, width = 4.5, height = 3, units = "in", compression="lzw")
...
I tried removing the M and M+ treatments (the treatments with data separation, no infection except for STD M+ has one infected animal). That model runs better and allows for including the block random effect. There is a sig dif in diet, but not clone or diet x clone.
...
pmaturesporemod <- lmer(log(mature_spores) ~ Diet * Clone + (1|Block), data = p_data_spores_mature)
...
pmaturesporemod2 <- lmer(log(mature_spores) ~ microcystin.avg + Diet + Clone + (1|Block), data = p_data_spores_mature)
pmaturesporemod2 <- lmer(mature_spores_log ~ microcystin.avg * Diet + (1|Block), data = p_data_spores)
#overdisp_fun(pmaturesporemod2)
summary(pmaturesporemod2)
Anova(pmaturesporemod2)
AIC(pmaturesporemod2)
plot(pmaturesporemod2)
qqnorm(resid(pmaturesporemod2))
qqline(resid(pmaturesporemod2))
...
p_data_spores_noM_tox$mature_spores_log <- log(p_data_spores_noM_tox$mature_spores+1)
pmaturesporemod3 <- lmer(log(mature_spores+1) ~ microcystin.avg * Diet + (1|Block), data = p_data_spores_noM_tox)
pmaturesporemod3 <- lmer(mature_spores_log ~ microcystin.avg * Diet * Clone + (1|Block), data = p_data_spores_noM_tox)
...
# Marginal effects from best model with Abundance on the x axis
past_maturespores2 <- ggpredict(pmaturesporemod2, c("microcystin.avg", "Diet", "Clone"))
...
p_data_spores$immature_spores_log <- log(p_data_spores$immature_spores+1)
p_data_spores_immature <- filter(p_data_spores, immature_spores > 0) # remove the one sample without mature spores
pimmaturesporemod <- lmer(immature_spores_log ~ Diet * Clone + (1|Block), data = p_data_spores)
...
m_data_spores[28, "microcystin.avg"] <- 14.58926751 # added in value of microcystin average for other M+ std metsch exposed and infected treatments
mmaturesporemod2 <- lmer(log(mature_spores+1) ~ microcystin.avg + Diet + (1|Block), data = m_data_spores)
summary(mmaturesporemod2)
Anova(mmaturesporemod3)
...
# Marginal effects from best model with Abundance on the x axis
metsch_maturespores2 <- ggpredict(mmaturesporemod2, c("microcystin.avg", "Diet"))
...
# figure with bodysize
p_feeding_fig <- plot(past_feed, dodge = 0.4, add.data = T) +
  scale_color_manual(values = diet_colors) +
...
# figure with bodysize
m_feeding_fig <- plot(metsch_feed, dodge = 0.4, add.data = T) +
  scale_color_manual(values = diet_colors) +
  ggtitle("Relative Feeding Rate (mL per hr per mm)") +
  labs(x = "Parasite Exposure", y = "Relative Feeding Rate (mL per hr per mm)") +
  theme_classic()
...
# NEED TO COME BACK TO FIX NAME OF METABOLIC RATE!!!!
...
# figure for presentation
p_resp_fig2 <- ggplot(p_data_resp, aes(x = Infection, y = metabloc.rate.Week1)) +
  geom_boxplot(aes(fill=Diet),position=position_dodge(width =0.9)) +
  geom_point(aes(color=Diet), size=2, position=position_jitterdodge(dodge.width=0.9, jitter.width = 0.2), alpha = 0.4) +
  facet_wrap(~Clone) +
  scale_x_discrete(limits = c("Uninfected", "Exposed", "Infected")) +
...
# Respiration rate during week 1 (exposure period) only
mrespmod <- lmer(metabolic.rate.Week1 ~ Diet + Infection * Clone +(1|Block), data = m_data_resp)
summary(mrespmod)
Anova(mrespmod)
AIC(mrespmod)
plot(mrespmod)
...
# Respiration rate over whole experiment
mrespmod2 <- lmer(metabolic.rate ~ Diet * Infection * Clone + Week + (1|Block), data = m_longdata_resp)
summary(mrespmod2)
Anova(mrespmod2)
AIC(mrespmod2)
plot(mrespmod2)
